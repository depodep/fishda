...
<style>
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
  ...
  .filter-select{background:var(--surface-2);border:1.5px solid var(--border);border-radius:7px;padding:6px 11px;font-size:11.5px;font-weight:600;color:var(--text-primary);font-family:'Mulish',sans-serif;outline:none;cursor:pointer;transition:border-color .18s;min-width:130px;}
  .filter-select:focus{border-color:var(--teal);}
  .filter-date{background:var(--surface-2);border:1.5px solid var(--border);border-radius:7px;padding:6px 11px;font-size:11.5px;color:var(--text-primary);font-family:'Mulish',sans-serif;outline:none;}
...
<div class="sidebar">
 ...
 <div class="main">
  ...
  <div class="filter-bar">
    ...
    <select class="filter-select" id="fBatch"><option value="">All Batches</option></select>
    <select class="filter-select" id="fSpecies">
      <option value="">All Species</option>
      <option>Tilapia</option><option>Bangus</option>
      <option>Galunggong</option><option>Tulingan</option>
    </select>
    <select class="filter-select" id="fMoisture">
    ...
    <input type="date" class="filter-date" id="fDateFrom" title="From">
    <span style="font-size:11px;color:var(--text-muted)">–</span>
    <input type="date" class="filter-date" id="fDateTo" title="To">
...
async function startSession(){
  ...
  try{
    ...
    const j=await(await fetch('session_api.php',{method:'POST',body:fd})).json();
    if(j.status==='success'){
      sessionRunning=true;
...
function updateControlUI(running,temp,hum){
  ...
  if(running){
    ...
    section.innerHTML=`<button class="btn-start" onclick="startSession()"><i class="fas fa-play me-2"></i>Start Drying Session</button>`;
...
async function pollSensorAlways(){
  try{
    const j=await(await fetch('controls_api.php?action=get_live_sensor')).json();
...
async function fetchMyRecords(){
  ...
  try{
    const j=await(await fetch('session_api.php?action=get_my_sessions')).json();
...
async function viewSessionDetail(sid){
  try{
    const j=await(await fetch(`session_api.php?action=get_session_logs&session_id=${sid}`)).json();
...
async function submitSchedule(){
  ...
  try{
    const j=await(await fetch('schedule_api.php',{method:'POST',body:fd})).json();
    if(j.status==='success'){
      closeScheduleModal();
...
async function deleteSchedule(id){
  ...
  await fetch('schedule_api.php',{method:'POST',body:fd});
  if(userCal) userCal.refetchEvents();
  loadUserSchedules();
}
...
function initUserCalendar(){
  ...
  userCal=new FullCalendar.Calendar(document.getElementById('userCalendar'),{
    ...
    events:async(info,success,failure)=>{
      try{ const r=await fetch('schedule_api.php?action=get_calendar_events'); const j=await r.json(); success(j.status==='success'?j.data:[]); }
      catch(e){ failure(e); }
    },
...
async function loadUserSchedules(){
  try{
    const j=await(await fetch(`schedule_api.php?action=get_my_schedules&proto_id=${PROTO_ID}`)).json();
...
async function pollPrototypeStatus(){
  try{
    const j=await(await fetch(`session_api.php?action=get_prototype_status&proto_id=${PROTO_ID}`)).json();
    if(j.status==='success'&&j.data){
